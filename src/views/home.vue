<template>
  <el-container class="home-container">
    <!--    导航-->
    <el-header>
      <div>
        <span style="margin-left:20px;">
          <img src="../assets/image/ilogo.png" width="250px;" style="margin-left:-10px;" height="100%;" alt srcset />
        </span>
      </div>
      <el-dropdown>
        <div class="block">
          <el-avatar :size="50" :src='avatar' style="cursor: pointer;"></el-avatar>
        </div>
        <el-dropdown-menu slot="dropdown" trigger="click">
          <el-dropdown-item>
            <span type="danger"><span class="el-icon-user"></span> &nbsp;{{userName}}</span>
          </el-dropdown-item>
          <el-dropdown-item>
            <span type="danger" @click="toWelcome"><span class="el-icon-house"></span> &nbsp;系统首页</span>
          </el-dropdown-item>
          <el-dropdown-item>

            <span type="danger" @click="logout"><span class="el-icon-switch-button"></span> &nbsp;退出登入</span>

          </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </el-header>
    <!--主体-->
    <el-container style="height: 500px;">
      <!--菜单-->
      <el-aside :width="isOpen==true?'20px':'200px'">
        <div class="toggle-btn" @click="toggleMenu">{{shouqi}}</div>
        <el-menu class="el-menu-vertical-demo" :collapse="isOpen" :router="true" :default-active="$route.path"
          background-color="#001529" :collapse-transition="false" text-color="rgba(255,255,255,0.7)" unique-opened>
          <MenuTree  :menuList="this.menuList"></MenuTree>
        </el-menu>
      </el-aside>
      <!--右边主体-->
      <el-main v-loading="loading">
        <router-view></router-view>
      </el-main>
    </el-container>
  </el-container>
</template>

<script>
  import MenuTree from "../components/menu/MenuTree.vue"; //引进菜单模板
  export default {
    data() {
      return {
        loading: true,
        activePath: "", //激活的路径
        isOpen: false,
        menuList: {},
        userName: "",
        avatar: require('../assets/image/avatar.jpg'),
        shouqi: "<<<"


      };
    },
    components: {
      MenuTree
    },
    methods: {
      toWelcome() {
        this.$router.push("/welcome");
      },
      /**
       *
       * 退出登入
       */
      async logout() {
        var res = await this.$confirm("此操作将退出系统, 是否继续?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).catch(() => {
          this.$message({
            type: "info",
            message: "已取消退出登入"
          });
        });
        if (res == "confirm") {
          this.submitOut()
        }
      },
      submitOut() {
        var name = this.userName;
        let storage = window.localStorage
        storage.removeItem('username')
        storage.removeItem('Authorization-token')
        this.$http.post('/api/user/logout', {
          userName: name
        }, {}).then((response) => {
          var ret = response.body
          if (ret.code === "6") {
            this.$message({
              message: ret.data,
              type: 'success',
              center: true,
              duration: 1000
            });
          } else {
            this.$message({
              message: '退出失败 ' + ret.code,
              type: 'warning',
              center: true,
              duration: 1000
            });
          }

        })
      },

      /**
        加载菜单数据
       */
      async getMenuList() {
        this.menuList = [{
            "id": 253,
            "parentId": 1,
            "menuName": "系统首页",
            "url": "/welcome",
            "icon": "el-icon-star-off",
            "orderNum": 1,
            "open": 0,
            "disabled": false,
            "perms": "welcome:view",
            "type": 0,
            "children": []
          },
          {
            "id": 11,
            "parentId": 1,
            "menuName": "所有展品",
            "url": "/allProducts",
            "icon": "el-icon-document",
            "orderNum": 1,
            "open": 0,
            "disabled": false,
            "perms": "",
            "type": 0,
            "children": []
          }, {
            "id": 12,
            "parentId": 1,
            "menuName": "展品入库",
            "url": "/addProducts",
            "icon": "el-icon-document-add",
            "orderNum": 2,
            "open": 0,
            "disabled": false,
            "perms": "",
            "type": 0,
            "children": []

          }, {
            "id": 13,
            "parentId": 1,
            "menuName": "展品出库",
            "url": "/warehouseout",
            "icon": "el-icon-document-remove",
            "orderNum": 1,
            "open": 0,
            "disabled": false,
            "perms": "",
            "type": 0,
            "children": []
          }, {

            "id": 310,
            "parentId": 1,
            "menuName": "展品编辑",
            "url": "/editinfo",
            "icon": "el-icon-edit",
            "orderNum": 1,
            "open": 0,
            "disabled": false,
            "perms": "",
            "type": 0,
            "children": []
          }, {
            "id": 269,
            "parentId": 1,
            "menuName": "展品位置",
            "url": "/position",
            "icon": "el-icon-location-information",
            "orderNum": 5,
            "open": 0,
            "disabled": false,
            "perms": "",
            "type": 0,
            "children": []
          }, {
            "id": 280,
            "parentId": 1,
            "menuName": "仓库展品",
            "url": "/warehouse",
            "icon": "el-icon-house",
            "orderNum": 5,
            "open": 0,
            "disabled": false,
            "perms": "",
            "type": 0,
            "children": []
          }
        ]
        return
        // const {
        //   data: res
        // } = await this.$http.get("user/findMenu");
        // if (res.code !== 200)
        //   return this.$message.error("获取菜单失败:" + res.msg);
        // this.menuList = res.data;
      },

      /**
       * 菜单伸缩
       */
      toggleMenu() {
        this.isOpen = !this.isOpen;
        this.shouqi === "<<<" ? this.shouqi = ">>>" : this.shouqi = "<<<"
      },



    },
    mounted() {

    },
    created() {
      let storage = window.localStorage
      this.userName = storage.getItem('username')
      //console.log("登陆了" + this.userName)
      if (this.userName === null || this.userName === "" || this.userName === undefined) {
        this.$router.push({
          path: '/login'
        })
      }
      //this.avatar= storage.getItem('avatar')
      this.getMenuList();
      // this.activePath = window.sessionStorage.getItem("activePath");
      // if (window.sessionStorage.getItem("activePath") == null) {
      //   this.activePath = '/welcome';
      // }
      setTimeout(() => {
        this.loading = false;
      }, 500);
    }
  };
</script>

<style>
  /* 为对应的路由跳转时设置动画效果 */

  .el-header {
    background-color: #001529;
    display: flex;
    justify-content: space-between;
    align-items: center;
    color: #fff;
    font-size: 19px;

    padding-left: 0px;
  }

  .el-aside {
    background-color: #001529
  }

  .el-main {
    background-color: #eaedf1;
  }

  .home-container {
    width: 100%;
    height: 100% !important;
  }

  .toggle-btn {
    background-color: #2d8cf0 !important;
    font-size: 10px;
    line-height: 24px;
    color: #fff;
    text-align: center;
    letter-spacing: 0.2em;
    cursor: pointer;
  }
</style>
