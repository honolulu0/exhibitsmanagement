<template>
  <div id="productCategroys">
    ...
    <el-breadcrumb separator="/" style="padding-left:10px;padding-bottom:10px;font-size:12px;">
      ...
      <el-breadcrumb-item>展品出库</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 右侧卡片区域 -->
    <el-card class="box-card">
      <el-row :gutter="6">
        ...
        <el-col :span="3" :style="currentName==='选中列表设置展品状态'?'color: #ff007f;':'color: #005CC5;'" style="text-decoration: underline; text-align:center;">
          <span>{{currentName}}</span>
        </el-col>
        <el-col :span="2">
          <el-select size="mini" v-model="addRuleForm.state" placeholder="请选择" @change='addNewChange'>
            <el-option v-for="item in stateList" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-col>
        <el-col :span="2">
          <el-button size="mini" type="success" icon="el-icon-edit" @click="update">设置</el-button>
        </el-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      addNewChange(val) {
        //console.log(this.addRuleForm.state)
        if (val === 'state') {
      ...
      handleclick(currentRow) {
        ...
        var a = JSON.parse(JSON.stringify(currentRow))
        this.addRuleForm.id = a.id
        this.addRuleForm.state = a.state
      ...
      update() {
        if (!(this.addRuleForm.id > 0)) {
          this.$message.error("请选择要设置的展品");
          return
        }
        if (this.addRuleForm.state===undefined || this.addRuleForm.state ==='' || this.addRuleForm.state === null) {
          this.$message.error("请选择要设置的展品状态");
          return
        }
        ...
        this.$http.post('/api/user/updataProdect',
          ...
          if (res.status == 200) {
            that.$message.success("展品状态设置成功");
            that.addRuleForm = {};

          } else {
            return that.$message.error("展品状态设置失败:" + res.msg);
          }
